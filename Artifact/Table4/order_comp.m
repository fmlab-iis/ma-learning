...
fun = 2;
%0 -> local; 1 -> web; 2 -> calculator; 3 -> missionary; 4 -> bdd;
...
%(1)find lamda and r
    ...
    TOTAL(1,rank) = time_mb;
    TOTAL(2,rank) = time_matrix;
    TOTAL(3,rank) = time_eq;
    TOTAL(4,rank) = time_counter;
    TOTAL(5,rank) = time_total;
    ...
    if(rank == print_iter)
        fprintf('Result of %.0f\n',rank);
        fprintf('[HM, TM, PAC,CE,OTHERS,TOTAL] = [%.2f,%.2f,%.2f,%.2f,%.2f,%.2f]\n',TOTAL(1,rank),TOTAL(2,rank),TOTAL(3,rank), TOTAL(4,rank),TOTAL(5,rank)-sum(TOTAL(1:4,rank)),TOTAL(5,rank));
         print_iter = print_iter*2;
    end
